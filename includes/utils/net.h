...
/**
 * Fill the `_out` parameter with the IP address of the input host
 */
extern void getHostnameIP(const char* _hostname, char* _out) __attribute__((nonnull));

/* Fill the `_addr` parameter with the IP address of the given input interface */
extern void getInterfaceIp(const char* _interface, char *_addr) __attribute__((nonnull));
 ...
 */
extern void addressNumberToString(u_int32_t _addr, char *_out, const bool _be) __attribute__((nonnull));
 ...
 */
extern u_int16_t computeChecksum(const unsigned char* buffer, const size_t size) __attribute__((nonnull));

/**
 * Performs Path MTU Discovery Search
 */
extern int pathMtuDiscovery(const char* _interface, const char* _hostname)  __attribute__((nonnull));
